...
export const AUTH_REGISTER = async (payload: { email: string, password: string, name: string, password_confirmation: string }) => {

    const { data, error } = await tryCatch(fetch(`${CONFIGS.BACKEND_BASE_URL}/api/auth/register`, {
        method: "POST",
        headers: {
